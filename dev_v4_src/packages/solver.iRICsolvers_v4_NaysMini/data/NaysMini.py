...
def nays2dh_init(cgnsName):
    ...
    common.write_fid = iric.cg_iRIC_Open(cgnsName, iric.CG_MODE_MODIFY)    #h230522 iRICv4
...
def main(cgnsName):
    ...
    ijh1 = iric.cg_iRIC_Read_Grid_Integer_Cell(common.write_fid, 'Obstacle')    #h230522 iRICv4
    ...
    while time<= etime:
        ...
        if icount%it_out==0:
            ...
            iric.cg_iRIC_Write_Sol_BaseIterative_Real(common.write_fid, "Discharge", qp)    #h230522 iRICv4
            nfile=nfile+1
            ...
            iric.cg_iRIC_Write_Sol_Node_Real(common.write_fid, "VelocityX", uxx)    #h230522 iRICv4
            vxx=np.reshape(vx,(ni*nj),order='F')
            iric.cg_iRIC_Write_Sol_Node_Real(common.write_fid, "VelocityY", vxx)    #h230522 iRICv4
            hsxx=np.reshape(hsx,(ni*nj),order='F')
            iric.cg_iRIC_Write_Sol_Node_Real(common.write_fid, "Depth", hsxx)    #h230522 iRICv4
            hxx=np.reshape(hx,(ni*nj),order='F')
            iric.cg_iRIC_Write_Sol_Node_Real(common.write_fid, "WaterSurface", hxx)    #h230522 iRICv4
            vorx=np.reshape(vor,(ni*nj),order='F')
            iric.cg_iRIC_Write_Sol_Node_Real(common.write_fid, "Vorticity", vorx)    #h230522 iRICv4
